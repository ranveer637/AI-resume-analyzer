...
// authFetch automatically attaches Bearer token if present
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 text-slate-100 p-4">
      <div className="w-full max-w-md bg-slate-800 border border-white/5 rounded-2xl p-6">
        ...
        <form onSubmit={submit} className="space-y-3 text-sm">
          ...
          <div>
            <label className="text-xs text-slate-300">Role (for guidance)</label>
            <select
              value={role}
            ...
            <div className="text-[11px] text-slate-400 mt-1">
              Select role you want to login as. If you do not have an account,
              use Register first.
            </div>
          </div>
  ...
  return (
    <Router>
      <Routes>
        ...
        {/* recruiter dashboard route referenced by header exists in other file */}
      </Routes>
    </Router>
  );
}
